...
const Contact = ({ darkMode }) => {
  ...
  const [interest, setInterest] = useState("select service");
  const [buttonText, setButtonText] = useState("Get in Touch");
  const [buttonBg, setButtonBg] = useState(darkMode ? "bg-white" : "bg-[#484E53]");
  const [buttonTextColor, setButtonTextColor] = useState(darkMode ? "text-black" : "text-white");

  useEffect(() => {
    setInterest("select service");
  }, []);

  const sendEmail = async (e) => {
    e.preventDefault();

    if (!name || !email || !message || !phone || interest === "select service") {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Change button text and background color to "Sent Successfully!"
        setButtonText("Sent Successfully");
        setButtonBg("bg-green-500");
        setButtonTextColor("text-white");

        // Reset button text and background color after 3 seconds
        setTimeout(() => {
          setButtonText("Get in Touch");
          setButtonBg(darkMode ? "bg-white" : "bg-[#484E53]");
          setButtonTextColor(darkMode ? "text-black" : "text-white");
        }, 3000);
      } else {
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                >
                  ...
                  <option value="Data Analytics & Visualization">
                    Data Analytics & Visualization
                  </option>
                  <option value="Predictive Analytics and Forecasting">
              ...
              <div className="md:pt-5 flex flex-col text items-center justify-center">
                <button
                  type="submit"
                  className={`text-md inter w-full rounded-lg px-3 py-2 md:px-5 md:py-3 font-bold ${buttonBg} ${buttonTextColor}`}
                >
                  {buttonText}
                </button>
              </div>
